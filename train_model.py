...
model_package = {
    ...
    'feature_importance': feature_importance,
     'sample_stats': {
        ...
        'avg_engagement_by_platform': df.groupby('platform')['engagement_rate'].mean().to_dict(),
        'avg_engagement_by_hour': df.groupby('hour')['engagement_rate'].mean().to_dict(),
...
import os
file_size_mb = os.path.getsize('social_media_model.joblib') / (1024 * 1024)
print(f"✅ Model saved as 'social_media_model.joblib' ({file_size_mb:.2f} MB)")

if file_size_mb > 10:
    print(f"⚠️  WARNING: File is still large ({file_size_mb:.2f} MB)")
else:
    print(f"✅ File size optimized!")
